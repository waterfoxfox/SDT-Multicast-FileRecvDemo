...
{														
	...
	//客户端SDK环境初始化
	SDTerminalMulticast_Enviroment_Init("./log", MULTICAST_TERMINAL_LOG_OUTPUT_LEVEL_INFO);
	...
	//用户类型
	MULTICAST_TERMINAL_USER_TYPE eUserType = MULTICAST_TERMINAL_TYPE_RECV_VIDEO_ONLY;
	...
	MulticastTerminalEncodeParams tEncodeParams;
	memset(&tEncodeParams, 0x0, sizeof(MulticastTerminalEncodeParams));
	//本业务场景下设置为2通道
	...
	MulticastTerminalLogonParams tLogonParams;
	memset(&tLogonParams, 0x0, sizeof(MulticastTerminalLogonParams));
	...
	MulticastTerminalTransParams tTransParams;
	memset(&tTransParams, 0x0, sizeof(MulticastTerminalTransParams));
	//本业务场景下开启组播NACK
	...
	tTransParams.unFecRedunMethod = MULTICAST_TERMINAL_FEC_TYPE_FIX;
