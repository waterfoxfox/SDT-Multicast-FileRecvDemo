...
CSDClient::CSDClient(MulticastTerminalEncodeParams *ptEncParams, MulticastTerminalLogonParams *ptLogonParams, MulticastTerminalTransParams *ptTransParams, void* pWindowHandle)
{
	m_bClosed = TRUE;

	memcpy(&m_tEncParams, ptEncParams, sizeof(MulticastTerminalEncodeParams));
	memcpy(&m_tLogonParams, ptLogonParams, sizeof(MulticastTerminalLogonParams));
	memcpy(&m_tTransParams, ptTransParams, sizeof(MulticastTerminalTransParams));

	//SDT-SDK
	m_pTerminal = SDTerminalMulticast_New(&m_tEncParams, RemoteVideoYuvData, RemoteAudioPcmData, this);
...
BOOL CSDClient::Start()
{
	...
	//创建音视频编解码资源并登录服务器
	bRet = SDTerminalMulticast_Online(m_pTerminal, &m_tLogonParams, &m_tTransParams);
	if (bRet == FALSE)
	{
		SDAudioRender_Stop(m_pAudioRender);
	...
	if (g_bSaveDownTs == TRUE)
	{
		...
		SDTerminalMulticast_StartCapDownStream(m_pTerminal, acSaveFileDir);
	}
	...
	if (g_bSaveDownRaw == TRUE)
	{
		...
		SDTerminalMulticast_StartCapDownRawStream(m_pTerminal, acSaveFileDir);
	}
...
void CSDClient::Stop()
{
	...
	if (g_bSaveDownTs == TRUE)
	{
		SDTerminalMulticast_StopCapDownStream(m_pTerminal);
	}

	if (g_bSaveDownRaw == TRUE)
	{
		SDTerminalMulticast_StopCapDownRawStream(m_pTerminal);
	}
